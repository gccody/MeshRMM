...
| 12: stale presence | Coordinator alarms retry presence publication from a durable outbox until the company's presence object accepts it. Inventory snapshots read that presence object and no longer query each coordinator (`server/tests/presence.mjs` asserts none are called), so a missed disconnect is repaired by the retried publication rather than by Refresh. Each publication carries a generation, and the presence object ignores older ones. |
