...
impl State {
    ...
    /// Negotiated public cipher identifier; never exposes session key material.
    pub async fn negotiated_cipher_suite(&self) -> Option<CipherSuiteId> {
        self.cipher_suite.lock().await.as_ref().map(|suite| suite.id())
    }
