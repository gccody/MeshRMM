...
or OIDC identity provider. **Add Agent** asks only for the installer platform
and downloads a company-authorized Windows setup executable. The computer name
comes from the endpoint, the device ID is generated by the server, and no Agent
record is created until setup redeems its authorization. The dashboard never
substitutes sample inventory or exposes a loose `agent.json`.
